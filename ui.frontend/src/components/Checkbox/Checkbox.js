...
export const Checkbox = ({ type, round , txtColor, inputColor, text, number, required, className }) => {
  const [checked, setChecked] = useState(true)
  if(round) {
    return ( //round checkbox
      <CheckBoxContainer className={className} role="checkbox" tabindex="0" title={text} onClick={() => setChecked(current => !current)} aria-label={type? checked : !checked}>
